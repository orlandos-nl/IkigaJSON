...
final class IkigaJSONTests: XCTestCase {
  ...
  func testEscapedUnicode() throws {
    do {
      let json: Data = #"{"simple":"\u00DF", "complex": "\ud83d\udc69\u200d\ud83d\udc69"}"#.data(
        using: .utf8)!
    ...
    do {
      let json: Data =
        #"{"simple":"\u00DFhello world", "complex": "\uD83D\uDC69\u200D\uD83D\uDC69\u200D\uD83D\uDC67\u200D\uD83D\uDC67hello world"}"#
        .data(using: .utf8)!
  ...
  private func measureTime(run block: () throws -> Void) rethrows -> TimeInterval {
    let date = Date()
  ...
  func testAllEncoding() throws {
    ...
    let primitives = AllPrimitives(
      s: "s", f: 3.1, d: 4.2, b: true, b2: false, i: 5, i8: 89, i16: -7667, i32: -889,
      i64: 123_123_213, u: 0x54, u8: 213, u16: 51, u32: 231, u64: 513, os: nil, of: nil, od: nil,
      ob: nil, ob2: nil, oi: nil, oi8: nil, oi16: nil, oi32: nil, oi64: nil, ou: nil, ou8: nil,
      ou16: nil, ou32: nil, ou64: nil)
    let primitives2 = AllPrimitives(
      s: "s", f: 3.1, d: 4.2, b: true, b2: false, i: 5, i8: 89, i16: -7667, i32: -889,
      i64: 123_123_213, u: 0x54, u8: 213, u16: 51, u32: 231, u64: 513, os: "s", of: 3.1, od: 4.2,
      ob: true, ob2: false, oi: -123, oi8: 12, oi16: -611, oi32: 1_231_512, oi64: 62341,
      ou: 3_152_113, ou8: 51, ou16: 314, ou32: 21321, ou64: 15_334_123_441)

    let test = Test(
      a: [primitives, primitives2], p: primitives, d: ["a": primitives, "b": primitives2])
    ...
    func objectNil() throws {
      func testFirst(against object: JSONObject?) {
        ...
        XCTAssertEqual(
          object.keys,
          [
            "s", "f", "d", "b", "b2", "i", "i8", "i16", "i32", "i64", "u", "u8", "u16", "u32",
            "u64",
          ])
      }
      ...
      func testSecond(against object: JSONObject?) {
        ...
        XCTAssertEqual(
          ...
          [
            "s", "f", "d", "b", "b2", "i", "i8", "i16", "i32", "i64", "u", "u8", "u16", "u32",
            "u64", "os", "of", "od", "ob", "ob2", "oi", "oi8", "oi16", "oi32", "oi64", "ou", "ou8",
            "ou16", "ou32", "ou64",
          ])
      }
  ...
  func testEmptyObjectKey() throws {
    // Test that empty object keys are handled correctly
    let json = """
      {"": "value"}
      """.data(using: .utf8)!
  ...
  func testDecodeArrayOfBool() throws {
    let data = zip(Array(repeating: true, count: 10), Array(repeating: false, count: 10)).flatMap {
      [$0, $1]
    }
    struct Foo: Codable {
      let bools: [Bool]
  ...
  struct NestedByUsingSuper: Codable {
    ...
    init(from decoder: Decoder) throws {
      ...
      self.unkeyed = try (
        unkeyedContainer.decode(Bool.self), .init(from: unkeyedContainer.superDecoder())
      )
    }
  ...
  func testSuperEncoderDecoderUsage() throws {
    let raw = NestedByUsingSuper(
      foo: 5, barSuper: "super bar", barParent: "parent bar", flag: true,
      barNumbered: "number your days")
  ...
  func testNilValueEncodingStrategies() throws {
    struct Foo: Codable, Equatable {
      ...
      init(from decoder: Decoder) throws {
        ...
        self.foo2 =
          try container.decodeNil(forKey: .foo2) ? nil : container.decode(Int.self, forKey: .foo2)
      ...
      func encode(to encoder: Encoder) throws {
        ...
        try self.foo2.map { try container.encode($0, forKey: .foo2) }
          ?? container.encodeNil(forKey: .foo2)
    ...
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob1), as: UTF8.self), #"{"foo1":0,"foo2":0,"foo3":0}"#)
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob2), as: UTF8.self), #"{"foo2":0,"foo3":0}"#)
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob3), as: UTF8.self),
      #"{"foo1":0,"foo2":null,"foo3":0}"#)
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob4), as: UTF8.self),
      #"{"foo1":0,"foo2":0,"foo3":null}"#)

    encoder.settings.nilValueEncodingStrategy = .alwaysEncodeNil
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob1), as: UTF8.self), #"{"foo1":0,"foo2":0,"foo3":0}"#)
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob2), as: UTF8.self),
      #"{"foo1":null,"foo2":0,"foo3":0}"#)
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob3), as: UTF8.self),
      #"{"foo1":0,"foo2":null,"foo3":0}"#)
    XCTAssertEqual(
    ...
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob2), as: UTF8.self), #"{"foo2":0,"foo3":0}"#)
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob3), as: UTF8.self), #"{"foo1":0,"foo3":0}"#)
    XCTAssertEqual(
      String(decoding: try encoder.encode(foob4), as: UTF8.self), #"{"foo1":0,"foo2":0}"#)

    var decoder = IkigaJSONDecoder()

    decoder.settings.nilValueDecodingStrategy = .default
    XCTAssertEqual(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo1":0,"foo2":0,"foo3":0}"#)),
      foob1)
    XCTAssertEqual(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo2":0,"foo3":0}"#)), foob2)
    XCTAssertEqual(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo1":0,"foo2":null,"foo3":0}"#)),
      foob3)
    XCTAssertEqual(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo1":0,"foo2":0,"foo3":null}"#)),
      foob4)
    ...
    XCTAssertEqual(
      ...
      foob1)
    XCTAssertEqual(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo1":null,"foo2":0,"foo3":0}"#)),
      foob2)
    XCTAssertEqual(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo1":0,"foo2":null,"foo3":0}"#)),
      foob3)
    ...
    XCTAssertThrowsError(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo2":0,"foo3":0}"#)))
    XCTAssertThrowsError(
      try decoder.decode(Foo.self, from: ByteBuffer(string: #"{"foo1":0,"foo3":0}"#)))
    XCTAssertThrowsError(
