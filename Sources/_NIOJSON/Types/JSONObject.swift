...
internal func equateJSON(_ lhs: JSONValue?, _ rhs: JSONValue?) -> Bool {
  ...
  case (let lhs as Bool, let rhs as Bool):
    return lhs == rhs
  case (let lhs as JSONObject, let rhs as JSONObject):
    return lhs == rhs
  case (let lhs as JSONArray, let rhs as JSONArray):
    return lhs == rhs
  case (.none, .none):
    return true
  default:
  ...
  CustomStringConvertible
{
  ...
  public var keys: [String] {
    return unsafe jsonBuffer.withSpan { span in
  ...
  private init(descriptionSize: Int) {
    ...
    description.complete(
      context,
      withResult: JSONToken.ObjectEnd(
  ...
  internal mutating func removeValue(index: Int, offset: Int) {
    ...
    jsonDescription.removeObjectDescription(
      atKeyIndex: offset, jsonOffset: Int(keyBounds.offset), removedJSONLength: Int(bounds.length))
  }
  ...
  fileprivate func value(forKey key: String, in span: Span<UInt8>) -> JSONValue? {
    ...
    case .object, .array:
      ...
      let subBuffer = jsonBuffer.getSlice(
        at: Int(jsonBounds.offset), length: Int(jsonBounds.length))!
  ...
  public mutating func updateValue(_ newValue: JSONValue?, forKey key: String) -> Bool {
    let keyResult = unsafe jsonBuffer.withSpan { span in
      return jsonDescription.keyOffset(forKey: key, convertingSnakeCasing: false, in: span)
    }
  ...
  public subscript(key: String) -> JSONValue? {
    ...
    set {
      ...
      defer {
        ...
        jsonDescription.incrementObjectCount(
          jsonSize: addedJSON, atValueIndexOffset: valueIndexOffset)
        jsonBuffer.writeInteger(UInt8.curlyRight)
      }

      switch newValue {
      case var object as JSONObject:
        self.jsonDescription.addNestedDescription(
          object.jsonDescription, at: Int32(valueJSONOffset))
...
public struct JSONObjectIterator: IteratorProtocol {
  ...
  public mutating func next() -> (String, JSONValue)? {
    ...
    defer { index += 1 }
    let key = keys[index]
    let value = unsafe object.jsonBuffer.withSpan { span in
      return object.value(forKey: key, in: span)
    }
...
// MARK: - ByteBuffer Span Extension

extension ByteBuffer {
  /// Provides span access to the buffer's readable bytes.
  /// This is an unsafe bridging function that creates a Span from the underlying buffer.
  @inlinable
  @unsafe
  func withSpan<T>(_ body: (Span<UInt8>) throws -> T) rethrows -> T {
    try unsafe self.withUnsafeReadableBytes { buffer in
      let span = unsafe Span<UInt8>(_unsafeElements: buffer.bindMemory(to: UInt8.self))
