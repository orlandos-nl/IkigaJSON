...
/// A type that manages encoder data storage using a safe Array.
///
...
final class SharedEncoderData {
  ...
  init(expectedSize: Int, expansionMode: ExpansionMode, settings: JSONEncoderSettings) {
    self.storage = [UInt8](repeating: 0, count: expectedSize)
  ...
  private func ensureCapacity(offset: Int, count: Int) {
    let needed = (offset &+ count) &- storage.count
    ...
    if needed > 0 {
      ...
      case .eager:
        newSize = max(storage.count &* 2, offset &+ count)
      case .normal:
      ...
      storage.append(contentsOf: repeatElement(0, count: newSize - storage.count))
    }
  }

  /// Inserts the byte into this storage
  func insert(_ byte: UInt8, at offset: inout Int) {
    ensureCapacity(offset: offset, count: 1)
    storage[offset] = byte
    offset = offset &+ 1
  }

  /// Inserts the other SharedEncoderData storage into this storage
  func insert(contentsOf other: SharedEncoderData, count: Int, at offset: inout Int) {
    ensureCapacity(offset: offset, count: count)
    for i in 0..<count {
      storage[offset &+ i] = other.storage[i]
    }
    offset = offset &+ count
  }

  /// Inserts the array bytes into this storage
  func insert(contentsOf data: [UInt8], at offset: inout Int) {
    ensureCapacity(offset: offset, count: data.count)
  ...
  /// Inserts the StaticString bytes into this storage
  func insert(contentsOf staticString: StaticString, at offset: inout Int) {
    // Convert StaticString to String and use its UTF8 view for safe access
  ...
  func cleanUp() {
    // No manual cleanup needed - Array manages its own memory
  }
}
...
public struct IkigaJSONEncoder: @unchecked Sendable {
  ...
  public func encode<E: Encodable>(_ value: E) throws -> Data {
    ...
    encoder.writeEnd()
    let data = Data(encoder.data.storage[..<encoder.offset])
    encoder.cleanUp()
    return data
  }
  ...
  public func encodeAndWrite<E: Encodable>(_ value: E, into buffer: inout ByteBuffer) throws {
    ...
    try value.encode(to: encoder)
    encoder.writeEnd()
    buffer.writeBytes(encoder.data.storage[..<encoder.offset])
    encoder.cleanUp()
  }
  ...
  public func encodeJSONObject<E: Encodable>(from value: E) throws -> JSONObject {
    ...
    buffer.writeBytes(encoder.data.storage[..<encoder.data.offset])
...
private final class _JSONEncoder: Encoder {
  ...
  init(userInfo: [CodingUserInfoKey: Any], settings: JSONEncoderSettings) {
    self.codingPath = []
    self.userInfo = userInfo
    data = SharedEncoderData(
      expectedSize: settings.expectedJSONSize, expansionMode: settings.bufferExpansionMode,
      settings: settings)
  }
  ...
  func writeOtherValue<T: Encodable>(_ value: T, forKey key: String? = nil) throws -> Bool {
    ...
    case let date as Date:
      ...
      case .custom(let custom):
        let offsetBeforeKey = offset
        let hadWrittenValue = didWriteValue
        if let key = key {
          writeKey(key)
        }
        ...
        if encoder.didWriteValue {
          didWriteValue = true
        } else if !encoder.didWriteValue, !encoder.didHaveContainers, key != nil,
          offset - offsetBeforeKey > 0
        {
  ...
  func writeValue<F: BinaryFloatingPoint & LosslessStringConvertible>(
    _ value: F, forKey key: String
  ) {
    writeKey(key)
    writeValue(value)
  }
...
private struct KeyedJSONEncodingContainer<Key: CodingKey>: KeyedEncodingContainerProtocol {
  ...
  mutating func encodeIfPresent(_ value: Bool?, forKey key: Key) throws {
    if encoder.shouldEmit(value) { try self.encode(value, forKey: key) }
  }
  ...
  mutating func encode<T>(_ value: T, forKey key: Key) throws where T: Encodable {
    ...
    let offsetBeforeKey = self.encoder.offset
    let hadWrittenValue = self.encoder.didWriteValue
    self.encoder.writeKey(key.stringValue)

    let encoder = _JSONEncoder(
      codingPath: codingPath + [key], userInfo: self.encoder.userInfo, data: self.encoder.data)
    try value.encode(to: encoder)
  ...
  mutating func superEncoder() -> Encoder {
    ...
    return _JSONEncoder(
      codingPath: codingPath + [SuperCodingKey.super], userInfo: self.encoder.userInfo,
...
private struct SingleValueJSONEncodingContainer: SingleValueEncodingContainer {
  let encoder: _JSONEncoder
  var codingPath: [CodingKey] { encoder.codingPath }
  ...
  mutating func encode<T>(_ value: T) throws where T: Encodable {
    ...
    let encoder = _JSONEncoder(
      codingPath: codingPath, userInfo: self.encoder.userInfo, data: self.encoder.data)
...
private struct UnkeyedJSONEncodingContainer: UnkeyedEncodingContainer {
  let encoder: _JSONEncoder
  var codingPath: [CodingKey] { encoder.codingPath }
  var count = 0
  ...
  mutating func encode<T>(_ value: T) throws where T: Encodable {
    ...
    } else if !encoder.didWriteValue, !encoder.didHaveContainers,
      self.encoder.offset - offsetBeforeComma > 0
    {
